...
# Overlay the stock suricata.yaml. Suricata 8 REPLACES (does not merge) the
# config when a second -c file is given, so without this include the entire
# base config — rule-files, default-rule-path, vars (HOME_NET/EXTERNAL_NET),
# classification — is silently dropped and the IDS loads 0 signatures.
