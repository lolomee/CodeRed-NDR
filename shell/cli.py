...
# SELF-CONTAINED config. Suricata 8 REPLACES (does not merge) the config when a
# second -c file is given, so this override must carry everything detection
# needs. (An `include:` of the base does NOT work reliably: Suricata replaces
# af-packet but MERGES the outputs list, so eve-log silently reverts to the
# base path /var/log/suricata/eve.json instead of /nsm — events never reach the
# forwarder.) Carrying vars + rule paths + classification here keeps it
...
def _parse_interval_seconds(spec: str) -> int:
    """Parse systemd-style time like '6h', '30m', '1d' to seconds. Returns 0 on bad input."""
    m = re.match(r'^([0-9]+)([smhd])$', spec.strip().lower())
    if not m:
        return 0
...
def _apply_auto_update_interval(interval: str, config: configparser.ConfigParser):
    """Persist a new auto-update interval to sensor.conf and a systemd drop-in
    for the timer. Drop-ins survive unit-file replacement via auto-update."""
    if not config.has_section('autoupdate'):
        config.add_section('autoupdate')
    config.set('autoupdate', 'interval', interval)
    save_config(config)

    drop_in_dir = '/etc/systemd/system/codered-update.timer.d'
    drop_in = f'{drop_in_dir}/override.conf'
    content = (
        '# Auto-generated by CLI option 20 — do not edit manually\n'
        '[Timer]\n'
        f'OnUnitActiveSec={interval}\n'
    )
    import tempfile as _tempfile
    with _tempfile.NamedTemporaryFile(mode='w', suffix='.conf', delete=False) as tf:
        tf.write(content)
        tmp = tf.name
    run_cmd(['mkdir', '-p', drop_in_dir], sudo=True)
    run_cmd(['cp', tmp, drop_in], sudo=True)
    run_cmd(['chmod', '0644', drop_in], sudo=True)
    os.unlink(tmp)
    run_cmd(['systemctl', 'daemon-reload'], sudo=True)
    rc, _ = run_cmd(['systemctl', 'is-active', 'codered-update.timer'], sudo=True)
    if rc == 0:
        run_cmd(['systemctl', 'restart', 'codered-update.timer'], sudo=True)


def auto_update_management():
    """Operator control of CodeRed NDR auto-update from GitHub.
    Shows state, toggles the systemd timer, changes cadence, and runs one-shot
    updates on demand."""
    audit('view:auto-update')

    while True:
        config = load_config()
        header('AUTO-UPDATE MANAGEMENT')

        rc_e, timer_enabled = run_cmd(['systemctl', 'is-enabled', 'codered-update.timer'], sudo=True)
        rc_a, timer_active = run_cmd(['systemctl', 'is-active', 'codered-update.timer'], sudo=True)
        interval = get_val(config, 'autoupdate', 'interval', '6h')
        branch   = get_val(config, 'autoupdate', 'branch', 'main')
        repo_url = get_val(config, 'autoupdate', 'repo_url',
                           'https://github.com/lolomee/CodeRed-NDR.git')

        last_update = '(never)'
        if os.path.exists('/var/log/codered/last-update.log'):
            try:
                last_update = open('/var/log/codered/last-update.log').read().strip()
            except OSError:
                pass

        # Display
        state_str = f'{timer_enabled.strip() or "?"} / {timer_active.strip() or "?"}'
        print_line('Timer state:', state_str)
        print_line('Interval:', interval)
        print_line('Branch:', branch)
        print_line('Repo:', repo_url)
        print_line('Last update:', last_update)

        # Next-fire info
        rc_n, out = run_cmd(['systemctl', 'list-timers', 'codered-update.timer',
                             '--no-pager'], sudo=True)
        for line in out.splitlines():
            if 'codered-update' in line and 'codered-update' != line.split()[-1]:
                parts = line.split()
                # NEXT and LEFT are first two fields when not "-"
                if parts[0] != '-':
                    print_line('Next fire:', ' '.join(parts[:3]))
                break

        # Currently-installed repo commit (for visibility)
        rc_c, commit = run_cmd(['git', '-C', '/opt/codered/repo', 'log',
                                '-1', '--format=%h %s'], sudo=True)
        if rc_c == 0:
            print_line('Repo HEAD:', commit.strip()[:80])

        print()
        print('  1. Enable auto-update  (start + enable timer)')
        print('  2. Disable auto-update (stop + disable timer)')
        print('  3. Change update interval')
        print('  4. Run one-shot update now')
        print('  5. Show recent update log (last 30 lines)')
        ...
        if choice == '1':
            rc, out = run_cmd(['systemctl', 'enable', '--now',
                               'codered-update.timer'], sudo=True)
            if rc == 0:
                audit('auto-update:enable')
                print('  Auto-update timer enabled and started.')
            else:
        ...
        if choice == '2':
            print('  Disabling auto-update means the sensor will NOT receive')
            print('  new detections, bug fixes, or security patches until you')
            print('  re-enable it or trigger a one-shot update manually.')
            if not confirm('Disable auto-update?', default=False):
                continue
            rc, out = run_cmd(['systemctl', 'disable', '--now',
                               'codered-update.timer'], sudo=True)
            if rc == 0:
                audit('auto-update:disable')
        ...
        if choice == '3':
            print('  Format: <number><unit> where unit is s / m / h / d')
            print('  Examples: 1h, 6h, 12h, 1d')
            print('  Minimum 1h enforced to avoid GitHub API rate limits.')
            def _v(s):
                return _parse_interval_seconds(s) >= 3600
            new_interval = prompt('New interval', interval, validator=_v, required=True)
            if not confirm(f'Set update interval to {new_interval}?'):
                continue
            _apply_auto_update_interval(new_interval, config)
            audit(f'auto-update:interval={new_interval}')
            print(f'  Interval set to {new_interval} via drop-in.')
            print(f'  (Drop-in: /etc/systemd/system/codered-update.timer.d/override.conf)')
            pause()
            continue

        if choice == '4':
            print('  Running one-shot update — this may take 30-60 seconds.')
            print('  (Network: git pull + file syncs)')
            print()
            rc, out = run_cmd(['systemctl', 'start', 'codered-update.service'],
                              sudo=True, timeout=600)
            audit('auto-update:manual-run')
            rc2, log = run_cmd(['tail', '-20', '/var/log/codered/update.log'],
                               sudo=True)
            print('  --- update log (last 20 lines) ---')
            for line in (log or '').strip().splitlines()[-20:]:
                print(f'  {line}')
            pause()
            continue

        if choice == '5':
            rc, log = run_cmd(['tail', '-30', '/var/log/codered/update.log'],
                              sudo=True)
            print()
            if log.strip():
                ...
                print('  No update log entries yet.')
            pause()
            continue
