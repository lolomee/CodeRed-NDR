...
if [ -n "${MONITOR_IF:-}" ]; then
    ...
    if [ "${CLOUD_MODE:-no}" = "yes" ]; then
        ...
        if [ -f "$LOCAL_ZEEK" ]; then
            # Enable the two VXLAN redef lines by removing leading comment characters
            python3 -c "
import re
with open('$LOCAL_ZEEK', 'r') as f:
    text = f.read()
# Between CLOUD_MODE_START and CLOUD_MODE_END markers, uncomment redef lines
def enable_block(m):
    block = m.group(0)
    block = re.sub(r'^# (redef)', r'', block, flags=re.MULTILINE)
    return block
text = re.sub(
    r'(# CLOUD_MODE_START.*?# CLOUD_MODE_END)',
    enable_block,
    text,
    flags=re.DOTALL
)
with open('$LOCAL_ZEEK', 'w') as f:
    f.write(text)
" 2>/dev/null && log "Zeek VXLAN decapsulation enabled" || warn "Could not update local.zeek — toggle manually via Menu -> 9"
        fi
