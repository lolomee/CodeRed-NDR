...
def run_setup():
    ...
    rc, out = run_cmd(['salt-call', '--local', 'state.apply', 'codered'], timeout=600, sudo=True)
    if rc != 0:
...
def show_status():
    ...
    print()
    print('  Services:')
    for svc in ['zeek', 'suricata', 'filebeat']:
    ...
    # Kernel hardening
    if os.path.exists('/etc/sysctl.d/99-codered-hardening.conf'):
        print_line('Kernel Hardening:', 'APPLIED')
    else:
        print_line('Kernel Hardening:', 'not applied')
...
def restart_services():
    """Restart, start, or stop sensor services."""
    header('SERVICE MANAGEMENT')
    print('  ── Restart ─────────────────────────────────')
    print('   1. Restart all (Zeek + Suricata + Filebeat)')
    print('   2. Restart Zeek')
    print('   3. Restart Suricata')
    print('   4. Restart Filebeat')
    print()
    print('  ── Start / Stop ────────────────────────────')
    ...
    print('   7. Start Zeek')
    print('   8. Stop Zeek')
    print('   9. Start Suricata')
    print('  10. Stop Suricata')
    print('  11. Start Filebeat')
    print('  12. Stop Filebeat')
    print()
    print('   0. Back')
    ...
    svc_actions = {
        '1':  ('Restart all services',  'restart', ['zeek', 'suricata', 'filebeat']),
        '2':  ('Restart Zeek',          'restart', ['zeek']),
        '3':  ('Restart Suricata',      'restart', ['suricata']),
        '4':  ('Restart Filebeat',      'restart', ['filebeat']),
        '5':  ('Start all services',    'start',   ['zeek', 'suricata', 'filebeat']),
        '6':  ('Stop all services',     'stop',    ['zeek', 'suricata', 'filebeat']),
        '7':  ('Start Zeek',            'start',   ['zeek']),
        '8':  ('Stop Zeek',             'stop',    ['zeek']),
        '9':  ('Start Suricata',        'start',   ['suricata']),
        '10': ('Stop Suricata',         'stop',    ['suricata']),
        '11': ('Start Filebeat',        'start',   ['filebeat']),
        '12': ('Stop Filebeat',         'stop',    ['filebeat']),
    }

    if choice in svc_actions:
        name, action, svc_list = svc_actions[choice]
        if confirm(f'{name}?'):
            audit(f'service:{action}:{",".join(svc_list)}')
            print(f'\n  {name}...')
            for svc in svc_list:
                print(f'    {action} {svc}...')
                rc, out = run_cmd(['systemctl', action, svc], timeout=120, sudo=True)
                if rc != 0:
...
def kernel_hardening():
    """Apply or remove kernel hardening."""
    header('KERNEL HARDENING')
    conf = '/etc/sysctl.d/99-codered-hardening.conf'
    is_applied = os.path.exists(conf)

    if is_applied:
        print('  Status: APPLIED')
        print()
        print('  Current hardening settings:')
        print('    - Block source-routed packets')
        print('    - SYN flood protection (TCP SYN cookies)')
        print('    - Ignore ICMP redirects')
        print('    - Log suspicious packets')
        print('    - Block broadcast ICMP')
        print('    - Full ASLR enabled')
        print('    - Kernel log access restricted')
        print('    - Kernel memory addresses hidden')
        print()
        print('  1. Remove kernel hardening')
        print('  0. Back')
        print()
        try:
            ...
            return
        if choice == '1' and confirm('Remove kernel hardening?'):
            audit('kernel-hardening:remove')
            run_cmd(['rm', '-f', conf], sudo=True)
            run_cmd(['sysctl', '--system'], timeout=30, sudo=True)
            print('  Kernel hardening removed.')
            pause()
    else:
        print('  Status: NOT APPLIED')
        print()
        print('  This will apply the following security settings:')
        print('    - Block source-routed packets (prevent path manipulation)')
        print('    - Enable SYN flood protection (TCP SYN cookies)')
        print('    - Ignore ICMP redirects (prevent MITM routing attacks)')
        print('    - Log suspicious packets (martian source addresses)')
        print('    - Block broadcast ICMP (prevent Smurf DDoS)')
        print('    - Enable full ASLR (memory exploit mitigation)')
        print('    - Restrict kernel log access (information leak prevention)')
        print('    - Hide kernel memory addresses (exploit mitigation)')
        print()
        print('  NOTE: These settings do NOT affect your network connectivity.')
        print()
        print('  1. Apply kernel hardening')
        print('  0. Back')
        print()
        try:
            choice = input('  Select: ').strip()
            ...
            return
        if choice == '1' and confirm('Apply kernel hardening?'):
            audit('kernel-hardening:apply')
            import tempfile
            with tempfile.NamedTemporaryFile(mode='w', suffix='.conf', delete=False) as f:
                f.write('# CodeRed NDR - Kernel Hardening\n')
                ...
                tmp_path = f.name
            run_cmd(['cp', tmp_path, conf], sudo=True)
            run_cmd(['chmod', '644', conf], sudo=True)
            os.unlink(tmp_path)
            run_cmd(['sysctl', '--system'], timeout=30, sudo=True)
            print('  Kernel hardening applied.')
            pause()
...
def run_diagnostics():
    ...
    for svc, proc in [('Zeek', 'zeek'), ('Suricata', 'suricata'), ('Filebeat', 'filebeat')]:
        rc, _ = run_cmd(['systemctl', 'is-active', svc], sudo=True)
        status = 'running' if rc == 0 else 'NOT RUNNING'
        print_line(f'    {svc}:', status)
...
def generate_support_bundle():
    ...
    journalctl -u zeek -u suricata -u filebeat --since "1 hour ago" --no-pager > $TMPDIR/journal.txt 2>/dev/null || true
    ...
    if rc == 0 and os.path.exists(bundle_path):
        ...
        print(f'  Download via: scp coderedai@<sensor-ip>:{bundle_path} .')
    else:
...
def change_password():
    """Change the coderedai user password."""
    ...
    # Use chpasswd via sudo
    rc, out = run_cmd(
        ['bash', '-c', f'echo "coderedai:{new_pw}" | chpasswd'],
        sudo=True
    )
    if rc == 0:
...
============================================================
  ...
  3. Power on, SSH in: ssh coderedai@<ip>
  ...
  Not receiving traffic?
    1. Menu -> 2 (Interfaces) -> check monitor shows PROMISC
  ...
  Services not running?
    1. Menu -> 9 (Service management) -> start/restart
...
def main_menu():
    ...
    while True:
        ...
        print('\n  -- Actions -------------------------------------')
        print('   9. Service management (start/stop/restart)')
        print('  10. Kernel hardening')
